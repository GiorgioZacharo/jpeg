...
void ChenIDct_f2r_forBody(int x[DCTSIZE2], int y[DCTSIZE2],
		register int i,
		register int *aptr,
...
void ChenIDct_f2r_forBody_s2e_forBody100Preheader(int x[DCTSIZE2], int y[DCTSIZE2]){
	register int i;
	register int *aptr;
	...
	reg_2:for (i = 0; i < 8; i++)
	{
	//	ChenIDct_f2r_forBody(x, y, i, aptr, a0, a1, a2, a3, b0, b1, b2, b3, c0, c1, c2, c3);
				aptr = x + i;
				b0 = LS (*aptr, 2);
				aptr += 8;
				a0 = LS (*aptr, 2);
...
void ChenIDct_f2r_forBody100(int y[DCTSIZE2],
		register int i,
		register int *aptr,
		register int a0, register int a1, register int a2, register int a3,
		register int b0, register int b1, register int b2, register int b3,
...
void ChenIDct_f2r_forBody100_s2e_vectorBodyPreheader(int y[DCTSIZE2]){
	register int i;
	register int *aptr;
	register int a0, a1, a2, a3;
	register int b0, b1, b2, b3;
	...
	reg_6:for (i = 0; i < 8; i++)
	{
		ChenIDct_f2r_forBody100(y, i, aptr, a0, a1, a2, a3, b0, b1, b2, b3, c0, c1, c2, c3);
		//		aptr = y + LS (i, 3);
		//		b0 = *(aptr++);
		//		a0 = *(aptr++);
		//		b2 = *(aptr++);
...
//		ChenIDct_f2r_vectorBody(i, aptr);
		*aptr = (((*aptr < 0) ? (*aptr - 8) : (*aptr + 8)) / 16);
}
/*
 *
...
//	ChenIDct_f2r_forBody_s2e_forBody100Preheader(x, y);

	  fun_01_a:for (i = 0; i < 8; i++)
	    {
		  aptr = x + i;
	      b0 = LS (*aptr, 2);
	      aptr += 8;
	...
	/* Loop over rows */
//	ChenIDct_f2r_forBody100_s2e_vectorBodyPreheader(y);

	fun_01_b:for (i = 0; i < 8; i++)
	{
		aptr = y + LS (i, 3);
		b0 = *(aptr++);
		a0 = *(aptr++);
	...
	//ChenIDct_f2r_vectorBody_s2e_forEnd212(y);
	
	  fun_01_c_a:for (i = 0, aptr = y; i < 64; i++, aptr++)
		fun_01_c_b:for (i = 0, aptr = y; i < 64; i++, aptr++)
			*aptr = (((*aptr < 0) ? (*aptr - 8) : (*aptr + 8)) / 16);
}
