...
IQuantize_f2r_forBody_s2e_forEndLoopexit(int matrix[DCTSIZE2], unsigned int qmatrix[DCTSIZE2], int * mptr){
	reg_12:for (mptr = matrix; mptr < matrix + DCTSIZE2; mptr++)
	{
		*mptr = *mptr * (*qmatrix);
		qmatrix++;
	}
...
void  BoundIDctMatrix_f2r_forBody_s2e_forEnd(int matrix[DCTSIZE2], int Bound){
	...
	reg_15:for (mptr = matrix; mptr < matrix + DCTSIZE2; mptr++)
	{
		if (*mptr < 0)
		{
			*mptr = 0;
...
void WriteOneBlock_f2r_ifThen7Us_s2e_forCond2ForEnd_crit_edgeUs(int store[DCTSIZE2], unsigned char out_buf[DCTSIZE2], int width, int hoffs, int diff){
	int e;
	reg_17:for (e = hoffs; e < hoffs + DCTSIZE; e++)
	{
		if (e < width)
		{
	...
	for (i = voffs; i < voffs + DCTSIZE; i++)
	{
		if (i < height)
		{
			...
			WriteOneBlock_f2r_ifThen7Us_s2e_forCond2ForEnd_crit_edgeUs(store, out_buf, width, hoffs, diff);
					//int e;
				//b:for (e = hoffs; e < hoffs + DCTSIZE; e++)
					//{
					//	if (e < width)
					//	{
					//		out_buf[diff + e] = (unsigned char) (*(store++));
...
//	WriteOneBlock_f2r_ifThenUs_s2e_forEnd13Loopexit(store, out_buf, width, height, voffs, hoffs);
	...
	reg_19:for (i = voffs; i < voffs + DCTSIZE; i++)
	{
		if (i < height)
		{
			int diff;
...
//	WriteOneBlock_f2r_entry_s2e_forEnd13(store, out_buf, width, height, voffs, hoffs);
	...
	/* Find vertical buffer offs. */
	fun_03:for (i = voffs; i < voffs + DCTSIZE; i++)
	{
		if (i < height)
		{
			int diff;
              ...
              short p_jinfo_image_width, short p_jinfo_image_height, int p_jinfo_MCUWidth)
{
  ...
  WriteOneBlock_f2r_WriteOneBlock (store1, p_out_buf, p_jinfo_image_width, p_jinfo_image_height, voffs, hoffs);
 ...
 WriteOneBlock_f2r_WriteOneBlock (store2, p_out_buf, p_jinfo_image_width, p_jinfo_image_height, voffs, hoffs);
  ...
  WriteOneBlock_f2r_WriteOneBlock (store3, p_out_buf, p_jinfo_image_width, p_jinfo_image_height, voffs, hoffs);
  ...
  WriteOneBlock_f2r_WriteOneBlock (store4, p_out_buf, p_jinfo_image_width, p_jinfo_image_height, voffs, hoffs);
