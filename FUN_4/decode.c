...
//	IQuantize_f2r_entry_s2e_forEnd(matrix, qmatrix);
	...
	fun_04:for (mptr = matrix; mptr < matrix + DCTSIZE2; mptr++)
		{
			*mptr = *mptr * (*qmatrix);
			qmatrix++;
		}
...
//	WriteOneBlock_f2r_entry_s2e_forEnd13(store, out_buf, width, height, voffs, hoffs);
	...
	/* Find vertical buffer offs. */
	fun_0:for (i = voffs; i < voffs + DCTSIZE; i++)
	{
		if (i < height)
		{
			int diff;
		...
		short p_jinfo_image_width, short p_jinfo_image_height, int p_jinfo_MCUWidth)
{
	...
	Write4Blocks_f2r_WriteOneBlockExit121_s2e_WriteOneBlockExit93(store3, p_out_buf,
			p_jinfo_image_width, p_jinfo_image_height, &voffs, &hoffs);
	/*
	 * XX
	 * OX
	...
	Write4Blocks_f2r_WriteOneBlockExit93_s2e_WriteOneBlockExit65(store4, p_out_buf,
			p_jinfo_image_width, p_jinfo_image_height, &voffs, &hoffs);
	/*
	 * XX
	 * XO
