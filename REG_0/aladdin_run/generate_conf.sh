...
while read PART TYPE ARRAY ARRAY_SIZE WORD_SIZE PART_FACTOR ; do
	...
	if [ $PART = "partition"  ]; then
		...
		echo  "$PART,$TYPE,$ARRAY,$ARRAY_SIZE,$WORD_SIZE,4" >> conf_ap_4
		echo  "$PART,$TYPE,$ARRAY,$ARRAY_SIZE,$WORD_SIZE,8" >> conf_ap_8
	else
		echo  "$PART,$TYPE,$ARRAY,$ARRAY_SIZE" >> conf_ap_2
		echo  "$PART,$TYPE,$ARRAY,$ARRAY_SIZE" >> conf_ap_4
		echo  "$PART,$TYPE,$ARRAY,$ARRAY_SIZE" >> conf_ap_8
	fi
...
for version in 2 4 8 ; do
	...
	cp conf_ap_"$version" conf_ap_"$version"_temp
	sed -i -- 's/,/ /g' conf_lu_"$version"_temp
	sed -i -- 's/,/ /g' conf_ap_"$version"_temp
done
...
for version in 2 4 8 ; do
	...
	while read ARG_1 ARG_2 ARG_3 ARG_4 ARG_5 ARG_6; do
        ...
        while read ARG_1 ARG_2 ARG_3 ARG_4 ARG_5 ARG_6; do
                ...
                if [ $ARG_1 = "unrolling"  ]; then
                        echo  "$ARG_1,$ARG_2,$ARG_3,4" >> conf_ap_"$version"_lu_4
                else
                        echo  "$ARG_1,$ARG_2,$ARG_3,$ARG_4,$ARG_5,$ARG_6" >> conf_ap_"$version"_lu_4
                fi

	done<conf_ap_"$version"_temp
